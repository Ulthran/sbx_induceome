...
def extract_peaks(file_path, output_image_fp, prominence=10, distance=50):
    ...
    peaks = detect_peaks(df['Coverage'].values, prominence=prominence, distance=distance)
    ...
    plt.plot(df['Position'], df['Coverage'], label='Coverage')
    plt.scatter(peak_positions['Position'], peak_positions['Coverage'], color='red', label='Peaks')
    plt.xlabel('Position')
    plt.ylabel('Coverage')
...
# Example usage
peak_data = extract_peaks(snakemake.input.pileup, snakemake.output.peaks_img, prominence=snakemake.params.prominence, distance=snakemake.params.distance)
